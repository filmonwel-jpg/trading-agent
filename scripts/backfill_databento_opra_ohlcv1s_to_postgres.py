...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download Databento OPRA ohlcv-1s history for a symbol list and store into Postgres"
    )
    ...
    parser.add_argument("--skip-opra-symbol-validation", action="store_true")
...
def _looks_like_opra_option_symbol(symbol: str) -> bool:
    normalized = symbol.replace(" ", "")
    return bool(re.search(r"\d{6,8}[CP]\d{1,8}$", normalized))


def _validate_credentials(api_key: str, user_id: str) -> tuple[str, str]:
...
def _validate_symbol_universe_for_opra(dataset: str, schema: str, stype_in: str, symbols: list[str]) -> None:
    if not dataset.upper().startswith("OPRA"):
        return
    if schema != "ohlcv-1s":
        return

    option_like = sum(1 for symbol in symbols if _looks_like_opra_option_symbol(symbol))

    if stype_in == "parent" and option_like > 0:
        sample = ", ".join([s for s in symbols if _looks_like_opra_option_symbol(s)][:5])
        raise ValueError(
            "When using stype_in='parent' for OPRA, provide parent roots / ROOT.OPT symbols (e.g., 'TSLA' or 'TSLA.OPT'). "
            f"Remove the OCC contract strings from your file. Found: {sample}"
        )
    elif stype_in == "raw_symbol" and option_like == 0:
        sample = ", ".join(symbols[:5])
        raise ValueError(
            "When using stype_in='raw_symbol' for OPRA, provide 21-character OCC option "
            f"contract symbols. Sample from file: {sample}"
        )
...
def _load_config(root_dir: Path, args: argparse.Namespace) -> BackfillConfig:
    ...
    if not args.dry_run:
        api_key, user_id = _validate_credentials(api_key, user_id)
    if not args.skip_opra_symbol_validation:
        _validate_symbol_universe_for_opra(args.dataset, args.schema, args.stype_in, symbols)
...
def _connect_db(root_dir: Path):
    ...
    conn_kwargs: dict[str, Any] = {
        "autocommit": True,
        "application_name": "databento_opra_ohlcv1s_backfill",
    }
...
def _ensure_table(conn) -> None:
    ddl = """
          CREATE TABLE IF NOT EXISTS databento_opra_ohlcv_1s (
          ...
          """
    idx = "CREATE INDEX IF NOT EXISTS idx_databento_opra_ohlcv_1s_symbol_time ON databento_opra_ohlcv_1s (symbol, ts_event)"
...
def _make_run_id(cfg: BackfillConfig) -> str:
    ...
    return f"opra-ohlcv1s-{stamp}-{sig}"
...
def _normalize_rows(queried_symbol: str, rows: Iterable[dict[str, Any]], cfg: BackfillConfig) -> list[tuple[Any, ...]]:
    out: list[tuple[Any, ...]] = []
    for row in rows:
        raw_ts = row.get("ts_event")
        if raw_ts is None:
            continue
        ...
        actual_symbol = str(row.get("symbol", queried_symbol))

        record_hash = _record_hash(actual_symbol, ts_event, row)
        out.append(
            (
                record_hash,
                actual_symbol,
...
def _insert_rows(conn, records: list[tuple[Any, ...]]) -> int:
    ...
    sql = """
          INSERT INTO databento_opra_ohlcv_1s (
              record_hash, symbol, ts_event, market_day, dataset, schema_name, stype_in,
              open_px, high_px, low_px, close_px, volume, payload, created_at, updated_at
          )
          VALUES (
                     %s, %s, %s, %s, %s, %s, %s,
                     %s, %s, %s, %s, %s, %s::jsonb, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
                 )
...
def _is_data_end_after_available_end(exc: Exception) -> bool:
    return "data_end_after_available_end" in str(exc).lower()


def _is_data_start_after_available_end(exc: Exception) -> bool:
    return "data_start_after_available_end" in str(exc).lower()
...
def _download_chunk_with_retry(
    ...
    api_symbol = symbol
    if cfg.stype_in == "parent" and cfg.dataset.upper().startswith("OPRA"):
        if not api_symbol.endswith(".OPT"):
            api_symbol = f"{api_symbol}.OPT"
    ...
    while True:
        try:
            ...
            print(
                f"[OPRA-OHLCV1S] retry symbol={api_symbol} chunk={chunk_start.date()}->{chunk_end.date()} "
...
def _download_symbol(cfg: BackfillConfig, client: db.Historical, conn, run_id: str, symbol: str) -> tuple[int, int, bool]:
    ...
    for chunk_start, chunk_end in _iter_ranges(cfg.start, cfg.end, cfg.chunk_days):
        if _pause_requested(cfg):
            print(f"[OPRA-OHLCV1S] pause requested via file: {cfg.pause_file}")
            paused = True
            break
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if is_start_error or available_end <= chunk_start:
                        print(
                            f"[OPRA-OHLCV1S] {symbol} reached dataset tail at {available_end.isoformat()} "
                    ...
                    print(
                        f"[OPRA-OHLCV1S] clamping {symbol} chunk end {effective_chunk_end.isoformat()} "
            ...
            if frame.empty:
                ...
                print(f"[OPRA-OHLCV1S] {symbol} {chunk_start.date()}->{chunk_end.date()} rows=0")
                continue
            ...
            print(f"[OPRA-OHLCV1S] {symbol} {chunk_start.date()}->{effective_chunk_end.date()} rows={inserted}")
        except Exception as exc:
            _mark_chunk_status(
                conn,
                run_id,
            ...
            if cfg.pause_on_network_error and _is_retryable_error(exc):
                print(f"[OPRA-OHLCV1S] network error, pausing run for resume later: {exc}")
                paused = True
                break
            raise
...
def main() -> int:
    ...
    print(f"[OPRA-OHLCV1S] symbols_file={cfg.symbols_file}")
    print(f"[OPRA-OHLCV1S] symbols_count={len(cfg.symbols)}")
    print(f"[OPRA-OHLCV1S] time_window={cfg.start.isoformat()} -> {cfg.end.isoformat()}")
    print(f"[OPRA-OHLCV1S] dataset={cfg.dataset} schema={cfg.schema} stype_in={cfg.stype_in}")
    print(f"[OPRA-OHLCV1S] databento_user_id={cfg.databento_user_id or '<unset>'}")
    print(f"[OPRA-OHLCV1S] pause_file={cfg.pause_file}")

    if cfg.dry_run:
        print("[OPRA-OHLCV1S] Dry run only. No Databento request and no DB writes were performed.")
        return 0
    ...
    print(f"[OPRA-OHLCV1S] run_id={run_id}")
    ...
    try:
        ...
        for idx, symbol in enumerate(cfg.symbols, start=1):
            print(f"[OPRA-OHLCV1S] ({idx}/{len(cfg.symbols)}) downloading {symbol}")
        ...
        print(f"[OPRA-OHLCV1S] FAILED run_id={run_id} error={exc}")
        raise
    finally:
        conn.close()

    final_status = "PAUSED" if paused else "COMPLETED"
    print(f"[OPRA-OHLCV1S] {final_status}. run_id={run_id} chunks={total_chunks} rows_upserted={total_rows}")
    return 0
