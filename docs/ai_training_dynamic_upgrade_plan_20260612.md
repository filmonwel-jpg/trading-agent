...
- The detailed step ledger is maintained in `docs/computer_capability_task_organization_20260613.md` using the required **Action plan** followed by **Action done** format for every step. The next controlled execution step is to run the OOF setup prediction generator on the fixed-quality 10-day baseline and perform a lifecycle/micro infrastructure smoke test with `--setup-predictions-csv`; do not start full-window builds or promotion work yet.
