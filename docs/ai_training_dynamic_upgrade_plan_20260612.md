...
- The detailed step ledger is maintained in `docs/computer_capability_task_organization_20260613.md` using the required **Action plan** followed by **Action done** format for every step. The next controlled execution step is a pre-build verification of the selected 10-day manifest and external-disk write path, then the partitioned 10-day pilot build contract, not full-window training.
