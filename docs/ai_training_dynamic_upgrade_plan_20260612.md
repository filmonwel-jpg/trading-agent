...
- The detailed step ledger is maintained in `docs/computer_capability_task_organization_20260613.md` using the required **Action plan** followed by **Action done** format for every step. The next controlled execution step is post-build artifact verification with `scripts/verify_databento_pilot_build.py`, not labels, model training, full-window builds, or promotion work.
