...
- Recommended next blocker: on the 48GB/write-capable computer, rerun the durable 10-day lifecycle/micro smoke with the updated trainer so the real fixed-quality artifacts include `calibration_manifest.json` and `calibration_reliability.csv`; then inspect Brier/ECE/reliability bins and decide whether post-hoc isotonic/Platt calibration is required before threshold-stability work. Separately, when market-hours data is available, complete the remaining Step 10 recorded-event replay artifact by capturing a real live-shaped NDJSON sample and replaying it through `DatabentoHistoricalStreamingBacktester`. Do not start paper/live promotion work from the 10-day smoke metrics, Step 10 parity wiring, or Step 11 raw calibration metrics alone.
...
- Lifecycle/micro probability-calibration metric artifacts now exist in branch code (`Brier`, `ECE`, reliability bins, `calibration_manifest.json`, and `calibration_reliability.csv`), but they measure raw RandomForest probabilities only. Until post-hoc calibration, frozen-holdout threshold stability, recorded-event replay parity, and paper/shadow drift gates pass, promising raw scorecards remain research-only.
